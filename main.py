...
# Autonomous dependency installer - ensures environment is functional on boot
def ensure_dependencies():
    required = ["docker", "openai", "google-generativeai", "httpx", "python-telegram-bot", "tenacity", "pyyaml"]
...
BOOTSTRAP_TASKS = [
    {
        ...
        "description": "Increment the version number in cradle/heartbeat.py from v0.6.0 to v0.6.1, commit the change, and push to GitHub. This proves you can successfully edit your own source code and deploy updates.\nNOTE: Because the sandbox is isolated, you MUST clone the repository, edit the files, commit, and push. Use this exact python script structure:\n```python\nimport subprocess, os\nimport re\n\nrepo_url = os.environ.get('CRADLE_GIT_URL', '')\n\n# Clone\nsubprocess.run(['git', 'clone', repo_url, '/tmp/cradle'], check=True)\n\n# Edit file\npath = '/tmp/cradle/cradle/heartbeat.py'\nwith open(path, 'r') as f:\n    content = f.read()\ncontent = content.replace('v0.6.0', 'v0.6.1')\nwith open(path, 'w') as f:\n    f.write(content)\n\n# Commit and push\nenv = {**os.environ, 'GIT_AUTHOR_NAME': 'Cradle Agent', 'GIT_AUTHOR_EMAIL': 'agent@cradle.ai', 'GIT_COMMITTER_NAME': 'Cradle Agent', 'GIT_COMMITTER_EMAIL': 'agent@cradle.ai'}\nsubprocess.run(['git', '-C', '/tmp/cradle', 'commit', '-am', 'feat: release v0.6.1'], check=True, env=env)\nsubprocess.run(['git', '-C', '/tmp/cradle', 'push', 'origin', 'main'], check=True)\nprint('SELF_UPDATE_PUSHED')\n```",
    },
]
